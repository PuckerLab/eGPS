...
def yr_compliance_check (tss, seq_per_contig, orientation):
	if orientation == '+':
		if ((seq_per_contig[tss - 2].upper() == 'T' or seq_per_contig[tss - 2].upper() == 'C') and (seq_per_contig[tss - 1].upper() == 'A' or seq_per_contig[tss - 1].upper() == 'G')):
			tss_yr_compliant = True
		else:
			tss_yr_compliant = False
	elif orientation == '-':
		if ((seq_per_contig[tss].upper() == 'A' or seq_per_contig[tss].upper() == 'G') and (seq_per_contig[tss - 1].upper() == 'T' or seq_per_contig[tss - 1].upper() == 'C')):
			tss_yr_compliant = True
		else:
			tss_yr_compliant = False
...
def extract_promoter_region( upstream_slice, downstream_slice, gene, start, end, tss, orientation, hard_cutoff, seq_per_contig, min_promoter_size, max_promoter_size, downstream_size ):
...
def main( arguments ):
	...
	other_tss_file = arguments[arguments.index('--tss_to_compare')]# full path to two column tab separated config file where column one is GOI and column two is the TSS for it from the source being compared against
	other_tss_dic = {}
	with open (other_tss_file, "r") as f:
		for line in f:
			parts = line.split().strip()
			other_tss_dic[parts[0]] = int(parts[1])
		...
		try:
			...
			if orientation == "+":	#only works on forward strand
				...
				if upstream_gene:
					...
					hard_cutoff = 1
				tss_list = {}
				tss_list[other_tool] = other_tss_dic[gene]
				...
				yr_compliance = yr_compliance_check(other_tss_dic[gene], seq_per_contig, orientation)
				for source in tss_list:
					if tss_list[source]:
						promoter_status, promoter, downstream_to_tss, full_seq = extract_promoter_region( upstream_slice, downstream_slice, gene, start, end, tss_list[source], orientation, hard_cutoff, seq_per_contig, min_promoter_size, max_promoter_size, downstream_size )
						full_seq_pos_strand_gene[gene][source] = full_seq
						promoter_status_dic[gene][source] = promoter_status
						promoter_dic[gene][source] = promoter

					else:
						full_seq_pos_strand_gene[gene][source] = None
						promoter_status_dic[gene][source] = None
						promoter_dic[gene][source] = None
				...
				for source in tss_list:
					if tss_list[source]:
						promoter_status, promoter, downstream_to_tss, full_seq = extract_promoter_region( upstream_slice, downstream_slice, gene, start, end, tss_list[source], orientation, hard_cutoff, seq_per_contig, min_promoter_size, max_promoter_size, downstream_size )
						full_seq_neg_strand_gene[gene][source] = full_seq
						promoter_status_dic[gene][source] = promoter_status
						promoter_dic[gene][source] = promoter
					else:
						full_seq_neg_strand_gene[gene][source] = None
						promoter_status_dic[gene][source] = None
						promoter_dic[gene][source] = None
	...
	with open( final_output_file, "w" ) as out:
		out.write("\t".join(
			["GeneID","TSS YR compliance","promoter status","promoter sequence","Additional comments"]) + "\n")
		for gene in list(results.keys()):
			final_results = []
			if gene in results:
				final_results.extend([gene,str(yr_compliance)])
