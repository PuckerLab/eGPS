...
__usage__ = """
					...
					Usage:
					python3 eGPS_other_tool_comparison.py
					--source_to_compare <Name of the tool or database providing alternate TSS annotation>
					--tss_to_compare <Two column tab-separated file with first column being gene name and second column being the TSS determined from the source taken for comparison>
...
def generate_plot( values, svalues, fig_file, atg_pos, cov_walk_start, boundary_tss_for_plot, other_tool, other_tool_tss_pos, basal_tss_pos, elevated_tss_pos, accelerated_tss_pos, genomic_start, genomic_end, gene, orientation, dna_sequence_for_plot, mrnas_to_plot, cds_to_plot, five_utr_to_plot, introns_to_plot ):
...
def run_fwd_analysis(other_tool, other_tss_dic, ks_pval, strength, lookahead, output_folder, pvalue,
	...
	try:
		generate_plot(values, svalues, fig_file, atg_pos, cov_walk_start, boundary_tss_for_plot, other_tool, other_tool_tss_pos, basal_tss_pos, elevated_tss_pos, accelerated_tss_pos, genomic_start, genomic_end, gene,orientation, dna_sequence_for_plot, mrnas_to_plot, cds_to_plot, five_utr_to_plot, introns_to_plot)
	except:
	...
	return {'TSS': walk_tss, 'start': start,'end': end}, walk_tss, other_tool_tss, basal_tss, elevated_tss, accelerated_tss, other_tool_tss_yr_compliant, basal_tss_yr_compliant, elevated_tss_yr_compliant, accelerated_tss_yr_compliant
...
def run_rev_analysis(other_tool, other_tss_dic, ks_pval, strength, lookahead, output_folder, pvalue,
	...
	return {'TSS': walk_tss, 'start': start,'end': end}, walk_tss, other_tool_tss, basal_tss, elevated_tss, accelerated_tss, other_tss_yr_compliant, basal_tss_yr_compliant, elevated_tss_yr_compliant, accelerated_tss_yr_compliant
...
def main( arguments ):
	...
	other_tool = arguments[arguments.index('--source_to_compare')+1]#specify epd tssfinder and so on
	other_tss_file = arguments[arguments.index('--tss_to_compare')+1]# full path to two column tab separated config file where column one is GOI and column two is the TSS for it from the source being compared against
	other_tss_dic = {}
	...
	for gene in goi:
		try:
			...
			if orientation == "+":	#only works on forward strand
				...
				result, walk_tss, other_tool_tss, basal_tss, elevated_tss, accelerated_tss, other_tool_tss_yr_compliant, basal_tss_yr_compliant, elevated_tss_yr_compliant, accelerated_tss_yr_compliant = run_fwd_analysis( other_tool, other_tss_dic, ks_pval, strength, lookahead, output_folder, background_percentage, intergenic_region_size, slide_step, intergenic_window_coverages, coverage_lookup, gene, cov_per_contig, scov_per_contig, seq_per_contig, start, end, fig_file, mincov, min_exon_size, hard_cutoff, flank_region_for_plot, tolerated_gap, splicesites, atg_pos, contig, genome_seq, gene_infos, genes_per_chromosome, mrna_infos, transcripts_per_gene, five_utr_infos, gene_atg_dic, cds_infos )
				tanalysis_end = time.perf_counter()
				tanalysis += (tanalysis_end - tanalysis_start)
				tss_compare_dic[gene] = [walk_tss, other_tool_tss, basal_tss, elevated_tss, accelerated_tss, other_tool_tss_yr_compliant, basal_tss_yr_compliant, elevated_tss_yr_compliant,accelerated_tss_yr_compliant]
				tss_list = {}
				if basal_tss:
				...
				tanalysis_start = time.perf_counter()
				result, walk_tss, other_tool_tss, basal_tss, elevated_tss, accelerated_tss, other_tss_yr_compliant, basal_tss_yr_compliant, elevated_tss_yr_compliant, accelerated_tss_yr_compliant = run_rev_analysis( other_tool, other_tss_dic, ks_pval, strength, lookahead, output_folder ,background_percentage, intergenic_region_size, slide_step, intergenic_window_coverages, coverage_lookup, gene, cov_per_contig, scov_per_contig, seq_per_contig, start, end, fig_file, mincov, min_exon_size, hard_cutoff, flank_region_for_plot, tolerated_gap, splicesites, atg_pos, contig, genome_seq, gene_infos, genes_per_chromosome, mrna_infos, transcripts_per_gene, five_utr_infos, gene_atg_dic, cds_infos )
				tanalysis_end = time.perf_counter()
				tanalysis += (tanalysis_end - tanalysis_start)
				tss_compare_dic[gene] = [walk_tss, other_tool_tss, basal_tss, elevated_tss, accelerated_tss, other_tss_yr_compliant, basal_tss_yr_compliant, elevated_tss_yr_compliant,accelerated_tss_yr_compliant]
				tss_list = {}
				if basal_tss:
					tss_list['basal'] = basal_tss
				else:
	...
	with open( final_output_file, "w" ) as out:
		out.write("\t".join(
			["GeneID", "Average gene coverage", "Gene expression level", "No.of isoforms",
			 "Walk TSS", f'{other_tool} TSS', "Basal TSS", "Elevated TSS","Accelerated TSS",
			 f'{other_tool} TSS YR compliance', "Basal TSS YR compliance", "Elevated TSS YR compliance","Accelerated TSS YR compliance",
		...
		for gene in list(results.keys()):
			...
			if gene in results:
				...
				str(tss_compare_dic[gene][4]), str(tss_compare_dic[gene][5]), str(tss_compare_dic[gene][6]), str(tss_compare_dic[gene][7]), str(tss_compare_dic[gene][8])])
	...
	if promoter_analysis == 'yes':
		with open (final_promoter_analysis_file, 'w') as out:
			out.write( "\t".join( [ f'{other_tool} promoter motif score',"Basal promoter motif score", "Elevated promoter motif score", "Accelerated promoter motif score",
									f'{other_tool} promoter motif score percentile',"Basal promoter motif score percentile", "Elevated promoter motif score percentile", "Accelerated promoter motif score percentile",
									f'{other_tool} promoter canonical hits',"Basal promoter canonical hits", "Elevated promoter canonical hits", "Accelerated promoter canonical hits",])+ "\n")
