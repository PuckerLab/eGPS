...
def generate_plot( epd_tss, values, svalues, fig_file, atg_pos, cov_walk_start, tss_pos, genomic_start, genomic_end, gene, orientation, dna_sequence_for_plot, mrnas_to_plot, cds_to_plot, five_utr_to_plot ):
	...
	ax2.plot([epd_tss, epd_tss], [0, max(svalues + values)], color="pink", linestyle="dotted",label="EPD TSS")  # EPD TSS position
	ax2.legend(loc='best')
	"""
	...
	ax_features.axvline(epd_tss, color="pink", linestyle="dotted")
...
def run_fwd_analysis( tss_dic, gene, cov_per_contig, scov_per_contig, seq_per_contig, start, end, fig_file, mincov, min_exon_size, hard_cutoff, flank_region_for_plot, tolerated_gap, splicesites, atg_genomic_pos, contig, genome_seq, gene_infos, genes_per_chromosome, mrna_infos, transcripts_per_gene, five_utr_infos, gene_atg_dic, cds_infos ):
	...
	epd_tss = tss_dic[gene]

	# extend right to include EPD TSS for + strand
	if epd_tss and epd_tss + flank_region_for_plot > plot_end_region:
		plot_end_region = epd_tss + flank_region_for_plot
	mrnas_to_plot = []
	cds_to_plot = []
	five_utr_to_plot = []
	...
	epd_tss = tss_dic[gene] - plot_start_region
	...
	try:
		generate_plot( epd_tss, values, svalues, fig_file, atg_pos, cov_walk_start, tss_pos, genomic_start, genomic_end, gene, orientation, dna_sequence_for_plot, mrnas_to_plot, cds_to_plot, five_utr_to_plot)
	except:
...
def run_rev_analysis( tss_dic, gene, cov_per_contig, scov_per_contig, seq_per_contig, start, end, fig_file, mincov, min_exon_size, hard_cutoff, flank_region_for_plot, tolerated_gap, splicesites, atg_genomic_pos, contig, genome_seq, gene_infos, genes_per_chromosome, mrna_infos, transcripts_per_gene, five_utr_infos, gene_atg_dic, cds_infos ):
	...
	# extend left to include EPD TSS for - strand
	if epd_tss and epd_tss - flank_region_for_plot < plot_start_region:
		plot_start_region = max(0, epd_tss - flank_region_for_plot)
...
def extract_promoter_region( gene, result, tss_dic, orientation, hard_cutoff, seq_per_contig, min_promoter_size, max_promoter_size ):
...
def promoter_motif_analysis (tss_dic, result, gene, orientation, promoter_seq, moods, pvalue, top_motifs, tss_prox, pfm_folder, tmp_folder, output_folder):
...
def main( arguments ):
	...
	tss_dic = {}
	if '--other_tss' in arguments:
		tss_file = arguments[arguments.index('--other_tss')+1]
		with open (tss_file, 'r') as f:
			for line in f:
				parts = line.strip().split()
				tss_dic[parts[0]] = int(parts[1])
		...
		try:
			...
			if orientation == "+":	#only works on forward strand
				...
				if upstream_gene:
					...
					hard_cutoff = 1
				result = run_fwd_analysis( tss_dic, gene, cov_per_contig, scov_per_contig, seq_per_contig, start, end, fig_file, mincov, min_exon_size, hard_cutoff, flank_region_for_plot, tolerated_gap, splicesites, atg_pos, contig, genome_seq, gene_infos, genes_per_chromosome, mrna_infos, transcripts_per_gene, five_utr_infos, gene_atg_dic, cds_infos )
				promoter_status, promoter = extract_promoter_region( gene, result, tss_dic, orientation, hard_cutoff, seq_per_contig, min_promoter_size, max_promoter_size )
				if promoter_analysis == 'yes':
					if os.path.exists(moods):
						best_motif_hits = promoter_motif_analysis(tss_dic, result, gene, orientation, promoter, moods, pvalue,top_motifs, tss_prox, pfm_folder, tmp_folder, output_folder)
				...
				result = run_rev_analysis( tss_dic, gene, cov_per_contig, scov_per_contig, seq_per_contig, start, end, fig_file, mincov, min_exon_size, hard_cutoff, flank_region_for_plot, tolerated_gap, splicesites, atg_pos, contig, genome_seq, gene_infos, genes_per_chromosome, mrna_infos, transcripts_per_gene, five_utr_infos, gene_atg_dic, cds_infos )
				promoter_status, promoter = extract_promoter_region( gene, result, tss_dic, orientation, hard_cutoff, seq_per_contig, min_promoter_size, max_promoter_size )
				if promoter_analysis == 'yes':
					if os.path.exists(moods):
						best_motif_hits = promoter_motif_analysis(tss_dic, result, gene, orientation, promoter, moods, pvalue, top_motifs,tss_prox, pfm_folder, tmp_folder, output_folder)
